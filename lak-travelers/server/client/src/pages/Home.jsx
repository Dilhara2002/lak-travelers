...
const Home = () => {
  ...
  // 👇 CORRECTED: Array of imported image variables
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      setCurrentImageIndex((prevIndex) => 
        // If current index is the last image, reset to 0, else increment
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans text-gray-800">
      ...
      {/* 1. MODERN HERO SECTION WITH ANIMATED SLIDER */}
      <div className="relative h-[600px] flex items-center justify-center px-4 overflow-hidden">
        ...
        <div className="absolute inset-0">
          {heroImages.map((imgUrl, index) => (
            <img 
              ...
              src={imgUrl} 
              alt={`Slide ${index}`} 
              // 👇 Animation Logic: duration-[2000ms] for slow smooth fade
              className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-[2000ms] ease-in-out ${
                index === currentImageIndex ? 'opacity-100 z-0' : 'opacity-0 -z-10'
              }`}
            />
          ))}
          {/* Gradient Overlay stays on top (z-10) */}
          <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/20 to-gray-900/90 z-10"></div>
        </div>

        {/* Hero Content (Text stays on top of everything) */}
        <div className="relative z-20 max-w-4xl w-full text-center text-white space-y-6 animate-fade-in-up">
          <span className="uppercase tracking-widest text-sm font-semibold bg-white/20 px-4 py-1 rounded-full backdrop-blur-md border border-white/30">
            Welcome to Paradise
          </span>
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight drop-shadow-2xl leading-tight">
          ...
          {/* Quick Action Search Bar Visual */}
          <div className="mt-8 bg-white/10 backdrop-blur-md p-2 rounded-full border border-white/20 max-w-xl mx-auto flex gap-2">
            <input 
              type="text" 
              placeholder="Where do you want to go?" 
              className="w-full bg-transparent text-white placeholder-gray-300 px-6 py-3 outline-none"
            />
            <Link to="/hotels" className="bg-yellow-500 hover:bg-yellow-400 text-gray-900 px-8 py-3 rounded-full font-bold transition shadow-lg flex items-center gap-2">
      ...
      <div className="container mx-auto px-4 py-20 space-y-24">
        ...
        <section>
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
            <div>
              <h2 className="text-4xl font-bold text-gray-900">Luxury Stays 🏨</h2>
              <p className="text-gray-500 mt-2 text-lg">Handpicked hotels for your perfect vacation.</p>
            </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center mt-6">
                      <div>
                        ...
                        <p className="text-blue-600 text-xl font-bold">LKR {hotel.pricePerNight.toLocaleString()}</p>
                      </div>
                      <Link to={`/hotels/${hotel._id}`} className="bg-gray-900 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-blue-600 transition shadow-md">
                        Details
                      </Link>
                    </div>
                  </div>
        ...
        <section>
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
            <div>
              <h2 className="text-4xl font-bold text-gray-900">Trending Adventures 🚐</h2>
              <p className="text-gray-500 mt-2 text-lg">Curated experiences you'll never forget.</p>
            </div>
        ...
        <div className="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={idx} className="flex flex-col items-center gap-2">
                <span className="text-4xl bg-gray-50 p-4 rounded-full">{item.icon}</span>
